...
import {Bot, CalendarDays, Globe2, Landmark, Map, Moon, MoreVertical, Route, Settings as SettingsIcon, ShieldCheck, Sparkles, Sun, WifiOff} from 'lucide-react';
...
const navLabelsByLanguage = {
  en: { home: 'Home', places: 'Places', itinerary: 'Itinerary', map: 'Map', events: 'Events' },
  es: { home: 'Inicio', places: 'Lugares', itinerary: 'Itinerario', map: 'Mapa', events: 'Eventos' },
  fr: { home: 'Accueil', places: 'Lieux', itinerary: 'Itinéraire', map: 'Carte', events: 'Événements' },
  de: { home: 'Start', places: 'Orte', itinerary: 'Route', map: 'Karte', events: 'Events' },
  it: { home: 'Inizio', places: 'Luoghi', itinerary: 'Itinerario', map: 'Mappa', events: 'Eventi' },
  pt: { home: 'Início', places: 'Lugares', itinerary: 'Roteiro', map: 'Mapa', events: 'Eventos' },
  ja: { home: 'ホーム', places: 'スポット', itinerary: '旅程', map: '地図', events: 'イベント' },
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <div className="flex flex-col items-end gap-1 sm:flex-row sm:items-center">
              <div className="flex items-center justify-end gap-1">
                ...
                <div className="relative"><${Globe2} className="absolute left-2 top-1/2 -translate-y-1/2 h-3 w-3 text-[hsl(var(--muted-foreground))]" /><select id="language" value=${lang} onChange=${e => setLanguage(e.target.value)} className="focus-ring min-h-[28px] w-[86px] rounded-[var(--radius-sm)] border border-[hsl(var(--border))] bg-[hsl(var(--card))] pl-6 pr-4 text-[11px] font-semibold leading-none">${languages.map(l => html`<option key=${l.code} value=${l.code}>${l.label}</option>`)}</select></div>
                <div className="relative" data-header-menu="true">
                  <button type="button" onClick=${() => setMenuOpen(!menuOpen)} aria-haspopup="menu" aria-expanded=${menuOpen} aria-label=${t('settings')} className="focus-ring grid h-8 min-h-0 w-8 place-items-center rounded-full bg-[hsl(var(--muted))] p-0 text-[hsl(var(--foreground))] overflow-hidden">
                    <${MoreVertical} className="h-3.5 w-3.5" />
                  </button>
                  ${menuOpen ? html`<div role="menu" className="absolute right-0 top-[calc(100%+.35rem)] z-50 w-52 overflow-hidden rounded-[var(--radius-lg)] border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-1.5 text-sm shadow-[var(--shadow-lg)]">
              ...
              <button onClick=${toggleDark} className="focus-ring grid h-8 min-h-0 w-8 place-items-center rounded-full bg-[hsl(var(--muted))] p-0 text-[hsl(var(--foreground))]" aria-label=${t('darkMode')}>${dark ? html`<${Sun} className="h-3.5 w-3.5" />` : html`<${Moon} className="h-3.5 w-3.5" />`}</button>
            </div>
          </div>
        </div>
      </header>
      <main className="mx-auto max-w-6xl px-3 py-4 md:py-6 overflow-x-hidden"><${Outlet} /></main>
      <nav className="bottom-nav fixed bottom-0 left-0 right-0 z-50 border-t border-[hsl(var(--border))] bg-[hsl(var(--card)/0.97)] backdrop-blur-xl md:hidden"><div className="grid grid-cols-5 gap-0.5 px-1.5 py-1.5">${nav.map(item => { const Icon = item.icon; return html`<${NavLink} key=${item.to} to=${item.to} className=${navClass}><${Icon} className="h-4 w-4" /><span className="text-[9px] leading-tight truncate max-w-full">${item.label}</span></${NavLink}>`; })}</div></nav>
      <nav className="bottom-nav hidden md:block lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-[hsl(var(--border))] bg-[hsl(var(--card)/0.97)] shadow-[0_-10px_28px_rgba(80,40,15,.08)] backdrop-blur-xl"><div className="mx-auto grid max-w-6xl grid-cols-5 gap-0.5 px-2 py-1.5">${nav.map(item => { const Icon = item.icon; return html`<${NavLink} key=${item.to} to=${item.to} className=${navClass}><${Icon} className="h-4 w-4" /><span className="text-[10px] leading-tight truncate max-w-full">${item.label}</span></${NavLink}>`; })}</div></nav>
